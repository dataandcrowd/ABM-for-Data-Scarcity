extensions [csv gis]
globals [data gu road lc districtPop districtadminCode no2_list station_list]
...
;;extract value from the list, using item 0 to remove the list, and just keep the value
  ...
  ;if ticks mod 2 = 0  [output-print (list item 2 entirelist one-of no2_list )] ;; even tick
  ;if ticks mod 2 != 0 [output-print (list item 2 entirelist
  ;  ((0.7 + random-float 0.3) * one-of no2_list ))] ;; odd tick and applying a discount in NO2
  ...
  ;ask patches with [monitor-code != false][
  ; if monitor-code = (item 3 data)
  ;]


  ;foreach data [ station ->
  ;  if item 3 station = "HR1" [output-print item ticks station]
  ;]
 ...
 ; foreach data [ station ->
 ;   ask patches with [monitor-code != false][
 ;     if monitor-code = (item 3 station)
 ;     [ let entirelist_ item ticks station
 ;       let no2_ sublist entirelist_ 5 29
 ;       set no2_ remove -999 no2_
 ;       set no2 no2_]
 ; ]
 ; ]
