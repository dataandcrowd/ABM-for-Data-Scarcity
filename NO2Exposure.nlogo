...
globals [
  ;;; Admin
  borough road IMD visit lc

  districtPop districtadminCode
  station_background station_road
  clock
  place-to-visit-age
  central
...
patches-own [is-research-area? is-road? name homecode traffic nox_weight is-built-area? central-London?
    visiting? visiting-borough?
  is-monitor-site? monitor-name monitor-code monitor-type nearest_station no2_list no2 IMDrank]

people-own  [health age districtName district-code
             homeName homePatch destinationName destinationPatch central-Londoner? weekend-shopper?
...
to get-exposed
  ask people [
    set no2-exposed [no2] of patch-here
  ]
...
to export-westminster-exposure
  let filename "westminster-no2-exposure.csv"
  ...
  ;; Append current exposures
  file-open filename
  ask people with [destinationName = "Westminster"] [
    file-print (word
      ticks ","      ; current tick
      who ","        ; turtle ID
      age ","        ; age of the person
